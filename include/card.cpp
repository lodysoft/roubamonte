...
const bool card::operator<(const card& crdOther)																	// Less than operator.
{
	if(this->figure() == crdOther.figure()) return this->suit() < crdOther.suit();

	return this->figure() < crdOther.figure();
}
